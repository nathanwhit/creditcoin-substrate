...
use sc_finality_grandpa::{LinkHalf, SharedVoterState};
...
pub fn new_full(mut config: Configuration, cli: Cli) -> Result<TaskManager, ServiceError> {
	...
	let (network, system_rpc_tx, tx_handler_controller, network_starter) =
		sc_service::build_network(sc_service::BuildNetworkParams {
			...
			warp_sync: None, // TODO: figure out if it's possible to use warp sync only after switched to pos
		})?;
	...
	CIDP::InherentDataProviders: sc_consensus_slots::InherentDataProviderExt + Send + Sync,
{
	...
	task_manager.spawn_essential_handle().spawn_blocking(
		"grandpa-voter",
		None,
		sc_finality_grandpa::run_grandpa_voter(grandpa_config).unwrap(),
	);
}
